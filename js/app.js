...
const renderProducts = (products) => {
  ...
  if (products.length === 0) {
    productGrid.innerHTML = '<p style="grid-column: 1/-1; text-align: center; padding: 4rem; color: var(--text-muted);">No products found matching your criteria.</p>';
    return;
  }
...
const setupEventListeners = () => {
  ...
  searchInput?.addEventListener('input', debounce((e) => {
    ...
    filterAndSortProducts(searchTerm, document.getElementById('category-filter').value, document.getElementById('sort-filter').value);
  }, 300));

  // Category filter
  const categoryFilter = document.getElementById('category-filter');
  categoryFilter?.addEventListener('change', (e) => {
    filterAndSortProducts(document.getElementById('search-input').value, e.target.value, document.getElementById('sort-filter').value);
  });

  // Sort filter
  const sortFilter = document.getElementById('sort-filter');
  sortFilter?.addEventListener('change', (e) => {
    filterAndSortProducts(document.getElementById('search-input').value, document.getElementById('category-filter').value, e.target.value);
  });
